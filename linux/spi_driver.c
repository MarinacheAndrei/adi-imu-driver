...
inline int spi_Init(adi_imu_SpiDevice_t *device) {
  ...
  if (device->fd < 0)
    DEBUG_PRINT_RET(Err_spi_InitFailed_e,
                    "[SPI]: Error: failed to open spi device %s\n",
                    device->dev);
  ...
  if (ret == -1)
    DEBUG_PRINT_RET(Err_spi_InitFailed_e,
                    "[SPI]: Error: Failed to set spi mode (wr). Error: %s\n",
                    strerror(errno));

  ret = ioctl(device->fd, SPI_IOC_RD_MODE, &device->mode);
  if (ret == -1)
    DEBUG_PRINT_RET(Err_spi_InitFailed_e,
                    "[SPI]: Error: Failed to set spi mode (rd). Error: %s\n",
                    strerror(errno));
  ...
  if (ret == -1)
    DEBUG_PRINT_RET(
        Err_spi_InitFailed_e,
        "[SPI]: Error: Failed to set bits per word (wr). Error: %s\n",
        strerror(errno));
  ...
  if (ret == -1)
    DEBUG_PRINT_RET(
        Err_spi_InitFailed_e,
        "[SPI]: Error: Failed to set bits per word (rd). Error: %s\n",
        strerror(errno));
  ...
  if (ret == -1)
    DEBUG_PRINT_RET(
        Err_spi_InitFailed_e,
        "[SPI]: Error: Failed to set max speed hz (wr). Error: %s\n",
        strerror(errno));
  ...
  if (ret == -1)
    DEBUG_PRINT_RET(
        Err_spi_InitFailed_e,
        "[SPI]: Error: Failed to set max speed hz (rd). Error: %s\n",
        strerror(errno));
...
inline int spi_ReadWrite(adi_imu_SpiDevice_t *device, const uint8_t *txBuf,
                         uint8_t *rxBuf, uint32_t xferLen, uint32_t numXfers,
                         uint32_t numRepeats, uint32_t enRepeatTx) {
  int ret = 0;
...
// #define DEBUG_SPI
      ...
      printf("[SPI TX]: ");
      if (enRepeatTx == 0)
        for (int i = 0; i < (xferLen * numXfers * numRepeats); i++)
          printf("%02X ", txBuf[i]);
      else
        for (int i = 0; i < (xferLen * numXfers); i++)
          printf("%02X ", txBuf[i]);
      printf("\n");
#endif
      ...
      if (ret < 1)
        DEBUG_PRINT_RET(Err_spi_RwFailed_e,
                        "[SPI]: Error: Failed to send spi message. Error: %s\n",
                        strerror(ret));

#ifdef DEBUG_SPI
      printf("[SPI RX]: ");
      for (int i = 0; i < (xferLen * total_xfers); i++)
        printf("%02X ", rxBuf[i]);
      printf("\n");
#endif
    }
