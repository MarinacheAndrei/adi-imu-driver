...
int adi_imu_Init(adi_imu_Device_t *pDevice) {
  ...
  if (pDevice->devType == IMU_HW_SPI &&
      ...
      pDevice->enable_buffer == IMU_FALSE) {
    DEBUG_PRINT("Warning: SPI clock out of range (%d Hz) (Setting to max speed "
                "= %d Hz)\n",
                pDevice->spiDev.speed, IMU_MAX_SPI_CLK);
    pDevice->spiDev.speed = IMU_MAX_SPI_CLK;
    if ((ret = hw_Init(pDevice)) < 0)
      return ret;
  }
  ...
  if (pDevice->devType == IMU_HW_SPI &&
      ...
      pDevice->enable_buffer == IMU_FALSE) {
    DEBUG_PRINT("Warning: SPI STALL time is low (%d us) (Setting to min stall "
                "time = %d us)\n",
                pDevice->spiDev.delay, IMU_MIN_STALL_US);
    pDevice->spiDev.delay = IMU_MIN_STALL_US;
  }
  ...
  if ((ret = hw_GetPage(pDevice)) < 0)
    return ret;

  /* read and verify product id */
  uint16_t prodId = 0x0000;
  if ((ret = hw_ReadReg(pDevice, REG_PROD_ID, &prodId)) < 0)
    return ret;
  if (prodId != pDevice->prodId) {
    DEBUG_PRINT(
        "Error: IMU product-id verification failed: Expected: %d, Read: %d.\n",
  ...
  if ((ret = hw_ReadReg(pDevice, REG_RANG_MDL, &(pDevice->rangeModel))) < 0)
    return ret;
...
static double getGyro16bitRes(adi_imu_Device_t *pDevice) {
  ...
  if (pDevice->prodId == 16465 || pDevice->prodId == 16467)
    defScale = (model == 0x3)   ? IMU_RES_GYRO16_46x1
               : (model == 0x7) ? IMU_RES_GYRO16_46x2
               : (model == 0xF) ? IMU_RES_GYRO16_46x3
                                : defScale;
  else if (pDevice->prodId == 16475 || pDevice->prodId == 16477)
    defScale = (model == 0x3)   ? IMU_RES_GYRO16_47x1
               : (model == 0x7) ? IMU_RES_GYRO16_47x2
               : (model == 0xF) ? IMU_RES_GYRO16_47x3
                                : defScale;
  else if (pDevice->prodId == 16495 || pDevice->prodId == 16497)
    defScale = (model == 0x3)   ? IMU_RES_GYRO16_49x1
               : (model == 0x7) ? IMU_RES_GYRO16_49x2
               : (model == 0xF) ? IMU_RES_GYRO16_49x3
                                : defScale;
  else if (pDevice->prodId == 16505 || pDevice->prodId == 16507)
    defScale = (model == 0x3)   ? IMU_RES_GYRO16_50x1
               : (model == 0x7) ? IMU_RES_GYRO16_50x2
               : (model == 0xF) ? IMU_RES_GYRO16_50x3
                                : defScale;
  else if (pDevice->prodId == 16545 || pDevice->prodId == 16547)
    defScale = (model == 0x3)   ? IMU_RES_GYRO16_54x1
               : (model == 0x7) ? IMU_RES_GYRO16_54x2
               : (model == 0xF) ? IMU_RES_GYRO16_54x3
                                : defScale;
  else
    defScale = 1.0;
  return defScale;
...
static double getGyro32bitRes(adi_imu_Device_t *pDevice) {
  ...
  if (pDevice->prodId == 16465 || pDevice->prodId == 16467)
    defScale = (model == 0x3)   ? IMU_RES_GYRO32_46x1
               : (model == 0x7) ? IMU_RES_GYRO32_46x2
               : (model == 0xF) ? IMU_RES_GYRO32_46x3
                                : defScale;
  else if (pDevice->prodId == 16475 || pDevice->prodId == 16477)
    defScale = (model == 0x3)   ? IMU_RES_GYRO32_47x1
               : (model == 0x7) ? IMU_RES_GYRO32_47x2
               : (model == 0xF) ? IMU_RES_GYRO32_47x3
                                : defScale;
  else if (pDevice->prodId == 16495 || pDevice->prodId == 16497)
    defScale = (model == 0x3)   ? IMU_RES_GYRO32_49x1
               : (model == 0x7) ? IMU_RES_GYRO32_49x2
               : (model == 0xF) ? IMU_RES_GYRO32_49x3
                                : defScale;
  else if (pDevice->prodId == 16505 || pDevice->prodId == 16507)
    defScale = (model == 0x3)   ? IMU_RES_GYRO32_50x1
               : (model == 0x7) ? IMU_RES_GYRO32_50x2
               : (model == 0xF) ? IMU_RES_GYRO32_50x3
                                : defScale;
  else if (pDevice->prodId == 16545 || pDevice->prodId == 16547)
    defScale = (model == 0x3)   ? IMU_RES_GYRO32_54x1
               : (model == 0x7) ? IMU_RES_GYRO32_54x2
               : (model == 0xF) ? IMU_RES_GYRO32_54x3
                                : defScale;
  else
    defScale = 1.0;
  return defScale;
...
int adi_imu_SetOutputDataRate(adi_imu_Device_t *pDevice, uint16_t outputRate) {
  ...
  if ((ret = hw_WriteReg(pDevice, REG_DEC_RATE, decRate)) < 0)
    return ret;
...
int adi_imu_GetDevInfo(adi_imu_Device_t *pDevice, adi_imu_DevInfo_t *pInfo) {
  ...
  if ((ret = hw_ReadReg(pDevice, REG_FNCTIO_CTRL, &(pInfo->fnctioCtrl))) < 0)
    return ret;

  /* read gpio ctrl io: Control, I/O pins, general-purpose */
  pDevice->spiDev.delay = IMU_STALL_US_GPIO_CTRL;
  if ((ret = hw_ReadReg(pDevice, REG_GPIO_CTRL, &(pInfo->gpioCtrl))) < 0)
    return ret;

  /* read clk cfg: Control, clock, and miscellaneous correction */
  pDevice->spiDev.delay = IMU_STALL_US_CONFIG;
  if ((ret = hw_ReadReg(pDevice, REG_CONFIG, &(pInfo->clkConfig))) < 0)
    return ret;

  /* read current decimation rate: Control, output sample rate decimation */
  pDevice->spiDev.delay = IMU_STALL_US_DEC_RATE;
  if ((ret = hw_ReadReg(pDevice, REG_DEC_RATE, &(pInfo->decimationRate))) < 0)
    return ret;

  /* read null cfg: Control, automatic bias correction configuration */
  pDevice->spiDev.delay = IMU_STALL_US_NULLCFG;
  if ((ret = hw_ReadReg(pDevice, REG_NULL_CNFG, &(pInfo->nullConfig))) < 0)
    return ret;

  /* read sync scale: Control, input clock scaling (PPS mode) */
  pDevice->spiDev.delay = IMU_STALL_US_SYNC_SCALE;
  if ((ret = hw_ReadReg(pDevice, REG_SYNC_SCALE, &(pInfo->syncScale))) < 0)
    return ret;

  /* read measurement range model identifier */
  pDevice->spiDev.delay = 100;
  if ((ret = hw_ReadReg(pDevice, REG_RANG_MDL, &(pInfo->gyroModelId))) < 0)
    return ret;

  /* read Filter bank 0 selection: Filter selection  */
  pDevice->spiDev.delay = IMU_STALL_US_FILTBNK0;
  if ((ret = hw_ReadReg(pDevice, REG_FILTR_BNK_0, &(pInfo->ftrBank0))) < 0)
    return ret;

  /* read Filter bank 1 selection: Filter selection  */
  pDevice->spiDev.delay = IMU_STALL_US_FILTBNK1;
  if ((ret = hw_ReadReg(pDevice, REG_FILTR_BNK_1, &(pInfo->ftrBank1))) < 0)
    return ret;

  /* read firmware revision */
  pDevice->spiDev.delay = 100;
  if ((ret = hw_ReadReg(pDevice, REG_FIRM_REV, &(pInfo->fwRev))) < 0)
    return ret;

  /* read firmware day/month */
  if ((ret = hw_ReadReg(pDevice, REG_FIRM_DM, &(pInfo->fwDayMonth))) < 0)
    return ret;

  /* read firmware year */
  if ((ret = hw_ReadReg(pDevice, REG_FIRM_Y, &(pInfo->fwYear))) < 0)
    return ret;

  /* read boot loader version */
  if ((ret = hw_ReadReg(pDevice, REG_BOOT_REV, &(pInfo->bootLoadVer))) < 0)
    return ret;

  /* read current page id */
  if ((ret = hw_ReadReg(pDevice, REG_PAGE_ID, &(pInfo->pageId))) < 0)
    return ret;

  /* read product id */
  if ((ret = hw_ReadReg(pDevice, REG_PROD_ID, &(pInfo->prodId))) < 0)
    return ret;

  /* read serial number */
  if ((ret = hw_ReadReg(pDevice, REG_SERIAL_NUM, &(pInfo->serialNumber))) < 0)
    return ret;
...
int adi_imu_PrintDevInfo(adi_imu_Device_t *pDevice, adi_imu_DevInfo_t *pInfo) {
  ...
  DEBUG_PRINT("IMU FW rev: %02x.%02x\n", IMU_FIRM_REV_UPPER(pInfo->fwRev),
              IMU_FIRM_REV_LOWER(pInfo->fwRev));
  DEBUG_PRINT("IMU FW date (MM-DD-YYYY): %02x-%02x-%x\n",
              IMU_FIRM_MONTH(pInfo->fwDayMonth),
              IMU_FIRM_DAY(pInfo->fwDayMonth), pInfo->fwYear);
  DEBUG_PRINT("IMU Bootloader ver: %d.%d\n",
              IMU_BOOT_REV_MAJOR(pInfo->bootLoadVer),
              IMU_BOOT_REV_MINOR(pInfo->bootLoadVer));
  DEBUG_PRINT("IMU Serial no: 0x%x\n", pInfo->serialNumber);
  DEBUG_PRINT("IMU Gyro Model: 0x%x [%s]\n", pInfo->gyroModelId,
              IMU_RANG_MDL(pInfo->gyroModelId));
...
int adi_imu_FindBurstPayloadIdx(const uint8_t *pBuf, unsigned bufLength,
                                unsigned *pPayloadOffset) {
  ...
  // DEBUG_PRINT("Error: burst frame invalid. Could not find BURST_ID pattern
  // (0xA5A5-0x0000).\n");
...
int adi_imu_ReadBurstRaw(adi_imu_Device_t *pDevice, uint8_t *pBuf,
  ...
  if ((ret = hw_SetPage(pDevice, pageId, 0)) < 0)
    return ret;

  /* send burst request and read response */
  /* as per ADIS16495 datasheet pg 7, its sufficient to send single 16-bit read
   * access to read whole burst unlike regular read */
  for (int i = 0; i < numBursts; i++)
    if ((ret = hw_ReadWriteRaw(pDevice, BURST_REQ, MAX_BRF_LEN_BYTES,
                               pBuf + MAX_BRF_LEN_BYTES * i,
                               MAX_BRF_LEN_BYTES)) < 0)
      return ret;
...
void init_crc32_table(void) {
  ...
  for (i = 0; i < 256; i++) {
    ...
    for (j = 0; j < 8; j++) {
      /* LSBit set? */
      if ((crc & (unsigned long)0x00000001) != (unsigned long)0) {
        /* process for bit set */
        crc = (crc >> 1) ^ P_32;
      } else {
...
unsigned long crc32_block(unsigned long crc, const unsigned short data[],
                          int n) {
...
int adi_imu_ReadBurst(adi_imu_Device_t *pDevice, uint8_t *pBuf,
                      uint32_t numBursts, adi_imu_BurstOutput_t *pData) {
  int ret = Err_imu_Success_e;
  unsigned pPayloadOffset = 0;
  if ((ret = adi_imu_ReadBurstRaw(pDevice, pBuf, numBursts)) < 0)
    return ret;
  ...
  for (uint32_t i = 0; i < numBursts; i++) {
    ...
    crc_calc = crc32_block(crc_calc, frame16, 15);
  ...
  for (int i = 0; i < numBursts; ++i)
    adi_imu_ScaleBurstOut_1(pDevice, pBuf + MAX_BRF_LEN_BYTES * i, IMU_TRUE,
                            IMU_TRUE, &pData[i]);
  return ret;
}

int adi_imu_CheckDiagStatus(adi_imu_Device_t *pDevice,
                            adi_imu_DiagStatus_t *pStatus) {
  int ret = Err_imu_Success_e;
  uint16_t status;

  /* read diagnostic status */
  if ((ret = hw_ReadReg(pDevice, REG_DIAG_STS, &status)) < 0)
    return ret;
  pStatus->data = status;
...
int adi_imu_CheckSysStatus(adi_imu_Device_t *pDevice,
                           adi_imu_SysStatus_t *pStatus) {
  int ret = Err_imu_Success_e;
  uint16_t status;

  /* read diagnostic status */
  if ((ret = hw_ReadReg(pDevice, REG_SYS_E_FLAG, &status)) < 0)
    return ret;
  pStatus->data = status;
  ...
  if (status & BITM_SYS_E_FLAG_SRAM_CRC) {
    ...
    DEBUG_PRINT("\nError: SRAM error condition."
                "CRC failure between the SRAM and flash memory. Initiate a "
                "reset to recover."
                "Replace the ADIS16495 if this error occurs. \n");
  }
  ...
  if (status & BITM_SYS_E_FLAG_SPI_COMM) {
    ...
    DEBUG_PRINT("\nError: SPI communication error."
                "The total number of SCLK cycles is not equal to an integer "
                "multiple of 16."
                "Repeat the previous communication sequence to recover."
                "Persistence in this error can indicate a weakness in the SPI "
                "service from the master processor. \n");
  }
  ...
  if (status & BITM_SYS_E_FLAG_SENSOR_TEST) {
    ...
    DEBUG_PRINT("\nError: Sensor failure."
                ...
                "Replace the ADIS16495 if the error persists, when it is "
                "operating in static inertial conditions. \n");
  ...
  if (status & BITM_SYS_E_FLAG_FLSH_MEM_UPD) {
    ...
    DEBUG_PRINT(
        ...
        "The most recent flash memory update failed (GLOB_CMD, Bit 3, see "
        "Table 142)."
  ...
  if (status & BITM_SYS_E_FLAG_SYNC_ERR) {
    ret = Err_imu_SystemError_e;
    DEBUG_PRINT("\nError: Sync error."
                "The sample timing is not scaling correctly, when operating in "
                "PPS mode (FNCTIO_CTRL Bit 8)."
                "Verify that the input sync frequency is correct and that "
                "SYNC_SCALE (see Table 154) has the correct value \n");
  }
  return ret;
}

int hw_ReadRegAccl(adi_imu_Device_t *pDevice,
                   adi_imu_AcclOutputRaw32_t *pData) {
  int ret = Err_imu_Success_e;
  uint16_t low, high;
  double res = getAccl32bitRes(pDevice);

  /* read x-axis LOW output */
  if ((ret = hw_ReadReg(pDevice, REG_X_ACCL_LOW, &low)) < 0)
    return ret;
  /* read x-axis HIGH output */
  if ((ret = hw_ReadReg(pDevice, REG_X_ACCL_OUT, &high)) < 0)
    return ret;
  pData->x = (int32_t)(((high << 16) & 0xFFFF0000) | low) * res;

  /* read y-axis LOW output */
  if ((ret = hw_ReadReg(pDevice, REG_Y_ACCL_LOW, &low)) < 0)
    return ret;
  /* read y-axis HIGH output */
  if ((ret = hw_ReadReg(pDevice, REG_Y_ACCL_OUT, &high)) < 0)
    return ret;
  pData->y = (int32_t)(((high << 16) & 0xFFFF0000) | low) * res;

  /* read z-axis LOW output */
  if ((ret = hw_ReadReg(pDevice, REG_Z_ACCL_LOW, &low)) < 0)
    return ret;
  /* read z-axis HIGH output */
  if ((ret = hw_ReadReg(pDevice, REG_Z_ACCL_OUT, &high)) < 0)
    return ret;
  pData->z = (int32_t)(((high << 16) & 0xFFFF0000) | low) * res;
  return ret;
}

int hw_ReadRegGyro(adi_imu_Device_t *pDevice,
                   adi_imu_GyroOutputRaw32_t *pData) {
  int ret = Err_imu_Success_e;
  uint16_t low, high;
  ...
  /* read x-axis LOW output */
  if ((ret = hw_ReadReg(pDevice, REG_X_GYRO_LOW, &low)) < 0)
    return ret;
  /* read x-axis HIGH output */
  if ((ret = hw_ReadReg(pDevice, REG_X_GYRO_OUT, &high)) < 0)
    return ret;
  pData->x = (((high << 16) & 0xFFFF0000) | low) * res;

  /* read y-axis LOW output */
  if ((ret = hw_ReadReg(pDevice, REG_Y_GYRO_LOW, &low)) < 0)
    return ret;
  /* read y-axis HIGH output */
  if ((ret = hw_ReadReg(pDevice, REG_Y_GYRO_OUT, &high)) < 0)
    return ret;
  pData->y = (((high << 16) & 0xFFFF0000) | low) * res;

  /* read z-axis LOW output */
  if ((ret = hw_ReadReg(pDevice, REG_Z_GYRO_LOW, &low)) < 0)
    return ret;
  /* read z-axis HIGH output */
  if ((ret = hw_ReadReg(pDevice, REG_Z_GYRO_OUT, &high)) < 0)
    return ret;
  pData->z = (((high << 16) & 0xFFFF0000) | low) * res;
  return ret;
}

int hw_ReadRegDelAng(adi_imu_Device_t *pDevice,
                     adi_imu_DelAngOutputRaw32_t *pData) {
  int ret = Err_imu_Success_e;
  uint16_t low, high;
  /* read x-axis LOW output */
  if ((ret = hw_ReadReg(pDevice, REG_X_DELTANG_LOW, &low)) < 0)
    return ret;
  /* read x-axis HIGH output */
  if ((ret = hw_ReadReg(pDevice, REG_X_DELTANG_OUT, &high)) < 0)
    return ret;
  pData->x = ((high << 16) & 0xFFFF0000) | low;

  /* read y-axis LOW output */
  if ((ret = hw_ReadReg(pDevice, REG_Y_DELTANG_LOW, &low)) < 0)
    return ret;
  /* read y-axis HIGH output */
  if ((ret = hw_ReadReg(pDevice, REG_Y_DELTANG_OUT, &high)) < 0)
    return ret;
  pData->y = ((high << 16) & 0xFFFF0000) | low;

  /* read z-axis LOW output */
  if ((ret = hw_ReadReg(pDevice, REG_Z_DELTANG_LOW, &low)) < 0)
    return ret;
  /* read z-axis HIGH output */
  if ((ret = hw_ReadReg(pDevice, REG_Z_DELTANG_OUT, &high)) < 0)
    return ret;
  pData->z = ((high << 16) & 0xFFFF0000) | low;
  return ret;
}

int hw_ReadRegDelVel(adi_imu_Device_t *pDevice,
                     adi_imu_DelVelOutputRaw32_t *pData) {
  int ret = Err_imu_Success_e;
  uint16_t low, high;
  /* read x-axis LOW output */
  if ((ret = hw_ReadReg(pDevice, REG_X_DELTVEL_LOW, &low)) < 0)
    return ret;
  /* read x-axis HIGH output */
  if ((ret = hw_ReadReg(pDevice, REG_X_DELTVEL_OUT, &high)) < 0)
    return ret;
  pData->x = ((high << 16) & 0xFFFF0000) | low;

  /* read y-axis LOW output */
  if ((ret = hw_ReadReg(pDevice, REG_Y_DELTVEL_LOW, &low)) < 0)
    return ret;
  /* read y-axis HIGH output */
  if ((ret = hw_ReadReg(pDevice, REG_Y_DELTVEL_OUT, &high)) < 0)
    return ret;
  pData->y = ((high << 16) & 0xFFFF0000) | low;

  /* read z-axis LOW output */
  if ((ret = hw_ReadReg(pDevice, REG_Z_DELTVEL_LOW, &low)) < 0)
    return ret;
  /* read z-axis HIGH output */
  if ((ret = hw_ReadReg(pDevice, REG_Z_DELTVEL_OUT, &high)) < 0)
    return ret;
  pData->z = ((high << 16) & 0xFFFF0000) | low;
  return ret;
}

int adi_imu_GetAcclScale(adi_imu_Device_t *pDevice,
                         adi_imu_AcclScale_t *pData) {
  int ret = Err_imu_Success_e;
  /* read x-axis scale output */
  if ((ret = hw_ReadReg(pDevice, REG_X_ACCL_SCALE, (uint16_t *)&(pData->x))) <
      0)
    return ret;

  /* read y-axis scale output */
  if ((ret = hw_ReadReg(pDevice, REG_Y_ACCL_SCALE, (uint16_t *)&(pData->y))) <
      0)
    return ret;

  /* read z-axis scale output */
  if ((ret = hw_ReadReg(pDevice, REG_Z_ACCL_SCALE, (uint16_t *)&(pData->z))) <
      0)
    return ret;
  return ret;
}

int adi_imu_GetGyroScale(adi_imu_Device_t *pDevice,
                         adi_imu_GyroScale_t *pData) {
  int ret = Err_imu_Success_e;
  /* read x-axis scale output */
  if ((ret = hw_ReadReg(pDevice, REG_X_GYRO_SCALE, (uint16_t *)&(pData->x))) <
      0)
    return ret;

  /* read y-axis scale output */
  if ((ret = hw_ReadReg(pDevice, REG_Y_GYRO_SCALE, (uint16_t *)&(pData->y))) <
      0)
    return ret;

  /* read z-axis scale output */
  if ((ret = hw_ReadReg(pDevice, REG_Z_GYRO_SCALE, (uint16_t *)&(pData->z))) <
      0)
    return ret;
  return ret;
}

int adi_imu_GetAcclBias(adi_imu_Device_t *pDevice,
                        adi_imu_AcclBiasRaw32_t *pData) {
  int ret = Err_imu_Success_e;
  uint16_t low, high;
  /* read x-axis LOW output */
  if ((ret = hw_ReadReg(pDevice, REG_XA_BIAS_LOW, &low)) < 0)
    return ret;
  /* read x-axis HIGH output */
  if ((ret = hw_ReadReg(pDevice, REG_XA_BIAS_HIGH, &high)) < 0)
    return ret;
  pData->x = ((high << 16) & 0xFFFF0000) | low;

  /* read y-axis LOW output */
  if ((ret = hw_ReadReg(pDevice, REG_YA_BIAS_LOW, &low)) < 0)
    return ret;
  /* read y-axis HIGH output */
  if ((ret = hw_ReadReg(pDevice, REG_YA_BIAS_HIGH, &high)) < 0)
    return ret;
  pData->y = ((high << 16) & 0xFFFF0000) | low;

  /* read z-axis LOW output */
  if ((ret = hw_ReadReg(pDevice, REG_ZA_BIAS_LOW, &low)) < 0)
    return ret;
  /* read z-axis HIGH output */
  if ((ret = hw_ReadReg(pDevice, REG_ZA_BIAS_HIGH, &high)) < 0)
    return ret;
  pData->z = ((high << 16) & 0xFFFF0000) | low;
  return ret;
}

int adi_imu_GetGyroBias(adi_imu_Device_t *pDevice,
                        adi_imu_GyroBiasRaw32_t *pData) {
  int ret = Err_imu_Success_e;
  uint16_t low, high;
  /* read x-axis LOW output */
  if ((ret = hw_ReadReg(pDevice, REG_XG_BIAS_LOW, &low)) < 0)
    return ret;
  /* read x-axis HIGH output */
  if ((ret = hw_ReadReg(pDevice, REG_XG_BIAS_HIGH, &high)) < 0)
    return ret;
  pData->x = ((high << 16) & 0xFFFF0000) | low;

  /* read y-axis LOW output */
  if ((ret = hw_ReadReg(pDevice, REG_YG_BIAS_LOW, &low)) < 0)
    return ret;
  /* read y-axis HIGH output */
  if ((ret = hw_ReadReg(pDevice, REG_YG_BIAS_HIGH, &high)) < 0)
    return ret;
  pData->y = ((high << 16) & 0xFFFF0000) | low;

  /* read z-axis LOW output */
  if ((ret = hw_ReadReg(pDevice, REG_ZG_BIAS_LOW, &low)) < 0)
    return ret;
  /* read z-axis HIGH output */
  if ((ret = hw_ReadReg(pDevice, REG_ZG_BIAS_HIGH, &high)) < 0)
    return ret;
  pData->z = ((high << 16) & 0xFFFF0000) | low;
  return ret;
}

int adi_imu_ConfigGpio(adi_imu_Device_t *pDevice, adi_imu_GPIO_e id,
                       adi_imu_Direction_e direction) {
  int ret = Err_imu_Success_e;
  DEBUG_PRINT("Configuring GPIO %d as %s...", id,
              (direction == IMU_INPUT) ? "INPUT" : "OUTPUT");
  uint32_t spidelay = pDevice->spiDev.delay;
  pDevice->spiDev.delay = IMU_STALL_US_GPIO_CTRL;
  if ((ret = hw_WriteReg(pDevice, REG_GPIO_CTRL, ((direction) << id))) < 0)
    return ret;
  pDevice->spiDev.delay = spidelay;
  DEBUG_PRINT("done.\n");
  return ret;
...
int adi_imu_SetGpio(adi_imu_Device_t *pDevice, adi_imu_GPIO_e id) {
  ...
  uint32_t spidelay = pDevice->spiDev.delay;
  pDevice->spiDev.delay = IMU_STALL_US_GPIO_CTRL;
  if ((ret = hw_ReadReg(pDevice, REG_GPIO_CTRL, &data)) < 0)
    return ret;
  if ((ret = hw_WriteReg(pDevice, REG_GPIO_CTRL,
                         data | (BITM_GPIO_CTRL_DIO1_DATA << id))) < 0)
    return ret;
  pDevice->spiDev.delay = spidelay;
  return ret;
}
...
int adi_imu_GetGpio(adi_imu_Device_t *pDevice, adi_imu_GPIO_e id,
                    uint8_t *val) {
  int ret = Err_imu_Success_e;
  uint16_t data = 0x00;
  uint32_t spidelay = pDevice->spiDev.delay;
...
int adi_imu_ConfigDataReady(adi_imu_Device_t *pDevice, adi_imu_GPIO_e id,
                            adi_imu_Polarity_e polarity) {
  int ret = Err_imu_Success_e;
  DEBUG_PRINT("Configuring data ready...");
  uint16_t fnctio = 0x00;
  uint32_t spidelay = pDevice->spiDev.delay;
  pDevice->spiDev.delay = IMU_STALL_US_FNCTIO;
  if ((ret = hw_ReadReg(pDevice, REG_FNCTIO_CTRL, &fnctio)) < 0)
    return ret;

  fnctio &= ~(BITM_FNCTIO_CTRL_DATA_RDY_POL | BITM_FNCTIO_CTRL_DATA_RDY_DIO);
  fnctio |= ((id) << BITP_FNCTIO_CTRL_DATA_RDY_DIO) |
            ((polarity) << BITP_FNCTIO_CTRL_DATA_RDY_POL);

  if ((ret = hw_WriteReg(pDevice, REG_FNCTIO_CTRL, fnctio)) < 0)
    return ret;
  pDevice->spiDev.delay = spidelay;
  DEBUG_PRINT("done.\n");
  return ret;
...
int adi_imu_ConfigSyncClkMode(adi_imu_Device_t *pDevice,
                              adi_imu_ClockMode_e mode, adi_imu_EnDis_e clkEn,
                              adi_imu_EdgeType_e polarity,
                              adi_imu_GPIO_e inputGpio) {
  int ret = Err_imu_Success_e;
  ...
  fnctio &= ~(BITM_FNCTIO_CTRL_SYNC_CLK_MODE | BITM_FNCTIO_CTRL_SYNC_CLK_EN |
              BITM_FNCTIO_CTRL_SYNC_CLK_POL | BITM_FNCTIO_CTRL_SYNC_CLK_DIO);
  fnctio |= ((mode) << BITP_FNCTIO_CTRL_SYNC_CLK_MODE) |
            ((clkEn) << BITP_FNCTIO_CTRL_SYNC_CLK_EN) |
            ((polarity) << BITP_FNCTIO_CTRL_SYNC_CLK_POL) |
            ((inputGpio) << BITP_FNCTIO_CTRL_SYNC_CLK_DIO);
...
int adi_imu_SetDataReady(adi_imu_Device_t *pDevice, adi_imu_EnDis_e val) {
  int ret = Err_imu_Success_e;
  DEBUG_PRINT("%s data ready...",
              (val == IMU_ENABLE) ? "Enabling" : "Disabling");
  uint16_t fnctio = 0x00;
  uint32_t spidelay = pDevice->spiDev.delay;
  pDevice->spiDev.delay = IMU_STALL_US_FNCTIO;
  if ((ret = hw_ReadReg(pDevice, REG_FNCTIO_CTRL, &fnctio)) < 0)
  ...
  if ((ret = hw_WriteReg(pDevice, REG_FNCTIO_CTRL,
                         (val == IMU_ENABLE)
                             ? fnctio | BITM_FNCTIO_CTRL_DATA_RDY_EN
                             : fnctio)) < 0)
    return ret;
  pDevice->spiDev.delay = spidelay;
  DEBUG_PRINT("done.\n");
...
int adi_imu_SetLineargComp(adi_imu_Device_t *pDevice, adi_imu_EnDis_e val) {
  int ret = Err_imu_Success_e;
  DEBUG_PRINT("%s linear g compensation...",
              (val == IMU_ENABLE) ? "Enabling" : "Disabling");
  uint32_t spidelay = pDevice->spiDev.delay;
  pDevice->spiDev.delay = IMU_STALL_US_CONFIG;
  if ((ret = hw_WriteReg(pDevice, REG_CONFIG,
                         (val == IMU_ENABLE) ? BITM_CONFIG_LIN_G_COMP : 0x00)) <
      0)
    return ret;
  pDevice->spiDev.delay = spidelay;
  DEBUG_PRINT("done.\n");
...
int adi_imu_SetPPercAlignment(adi_imu_Device_t *pDevice, adi_imu_EnDis_e val) {
  int ret = Err_imu_Success_e;
  DEBUG_PRINT("%s point of percussion alignment...",
              (val == IMU_ENABLE) ? "Enabling" : "Disabling");
  uint32_t spidelay = pDevice->spiDev.delay;
  pDevice->spiDev.delay = IMU_STALL_US_CONFIG;
  if ((ret = hw_WriteReg(pDevice, REG_CONFIG,
                         (val == IMU_ENABLE) ? BITM_CONFIG_PNT_PERC_ALIGN
                                             : 0x00)) < 0)
    return ret;
  pDevice->spiDev.delay = spidelay;
  DEBUG_PRINT("done.\n");
...
int adi_imu_SoftwareReset(adi_imu_Device_t *pDevice) {
  ...
  if ((ret = hw_WriteReg(pDevice, REG_GLOB_CMD, BITM_GLOB_CMD_SOFT_RST)) < 0)
    return ret;
...
int adi_imu_ClearUserCalibration(adi_imu_Device_t *pDevice) {
  ...
  if ((ret = hw_WriteReg(pDevice, REG_GLOB_CMD, BITM_GLOB_CMD_CLR_USR_CALIB)) <
      0)
    return ret;
  DEBUG_PRINT("done.\n");
  return ret;
...
int adi_imu_UpdateFlashMemory(adi_imu_Device_t *pDevice) {
  ...
  if ((ret = hw_WriteReg(pDevice, REG_GLOB_CMD, BITM_GLOB_CMD_FLASH_MEM_UPD)) <
      0)
    return ret;
...
int adi_imu_PerformSelfTest(adi_imu_Device_t *pDevice) {
  ...
  if ((ret = hw_WriteReg(pDevice, REG_GLOB_CMD, BITM_GLOB_CMD_SELF_TEST)) < 0)
    return ret;
  ...
  if ((ret = adi_imu_CheckDiagStatus(pDevice, &diagStatus)) < 0)
    return ret;
...
int adi_imu_ConfigBiasCorrectionTime(adi_imu_Device_t *pDevice, uint8_t time) {
  ...
  uint32_t spidelay = pDevice->spiDev.delay;
  pDevice->spiDev.delay = IMU_STALL_US_NULLCFG;
  if ((ret = hw_WriteReg(pDevice, REG_NULL_CNFG, dat)) < 0)
    return ret;
  pDevice->spiDev.delay = spidelay;
  DEBUG_PRINT("Finished!\n");
  return ret;
...
int adi_imu_UpdateBiasCorrection(adi_imu_Device_t *pDevice) {
  ...
  if ((ret = hw_WriteReg(pDevice, REG_GLOB_CMD, BITM_GLOB_CMD_BIAS_CORR_UPD)) <
      0)
    return ret;
  DEBUG_PRINT("Finished!\n");
  return ret;
...
int adi_imu_SelectBiasConfigAxes(
    adi_imu_Device_t *pDevice, adi_imu_NullConfig_e XG, adi_imu_NullConfig_e YG,
  ...
  uint32_t spidelay = pDevice->spiDev.delay;
  pDevice->spiDev.delay = IMU_STALL_US_NULLCFG;
  if ((ret = hw_ReadReg(pDevice, REG_NULL_CNFG, &nullcnfg)) < 0)
    return ret;
  nullcnfg &=
      ~(BITM_NULL_CNFG_EN_XG | BITM_NULL_CNFG_EN_YG | BITM_NULL_CNFG_EN_ZG |
        BITM_NULL_CNFG_EN_XA | BITM_NULL_CNFG_EN_YA | BITM_NULL_CNFG_EN_ZA);
  nullcnfg |= ((XG) << BITP_NULL_CNFG_EN_XG) | ((YG) << BITP_NULL_CNFG_EN_YG) |
              ((ZG) << BITP_NULL_CNFG_EN_ZG) | ((XA) << BITP_NULL_CNFG_EN_XA) |
              ((YA) << BITP_NULL_CNFG_EN_YA) | ((ZA) << BITP_NULL_CNFG_EN_ZA);
  if ((ret = hw_WriteReg(pDevice, REG_NULL_CNFG, nullcnfg)) < 0)
    return ret;
  pDevice->spiDev.delay = spidelay;
  DEBUG_PRINT("Finished!\n");
  return ret;
...
int adi_imu_ParseBurstOut(adi_imu_Device_t *pDevice, const uint8_t *pBuf,
                          adi_imu_Boolean_e checkBurstID,
                          adi_imu_Boolean_e enByteSwap,
                          adi_imu_BurstOutputRaw_t *pRawData) {
  unsigned payloadOffset = 0;
  if (checkBurstID) {
    int ret =
        adi_imu_FindBurstPayloadIdx(pBuf, MAX_BRF_LEN_BYTES, &payloadOffset);
    if (ret < 0)
      return ret;
  }

  if (pDevice->devType == IMU_HW_SPI &&
      pDevice->spiDev.status == IMUBUF_SPI_READY) {
    pRawData->sysEFlag = IMU_BSWAP_16(*(uint16_t *)(pBuf + payloadOffset));
    pRawData->tempOut =
        (int16_t)(IMU_BSWAP_16(*(uint16_t *)(pBuf + payloadOffset + 2)));

    pRawData->gyro.x =
        (int32_t)(IMU_BSWAP_32(*(uint32_t *)(pBuf + payloadOffset + 4)));
    pRawData->gyro.y =
        (int32_t)(IMU_BSWAP_32(*(uint32_t *)(pBuf + payloadOffset + 8)));
    pRawData->gyro.z =
        (int32_t)(IMU_BSWAP_32(*(uint32_t *)(pBuf + payloadOffset + 12)));

    pRawData->accl.x =
        (int32_t)(IMU_BSWAP_32(*(uint32_t *)(pBuf + payloadOffset + 16)));
    pRawData->accl.y =
        (int32_t)(IMU_BSWAP_32(*(uint32_t *)(pBuf + payloadOffset + 20)));
    pRawData->accl.z =
        (int32_t)(IMU_BSWAP_32(*(uint32_t *)(pBuf + payloadOffset + 24)));

    pRawData->dataCntOrTimeStamp =
        (unsigned)IMU_BSWAP_16(*(uint16_t *)(pBuf + payloadOffset + 28));
    pRawData->crc =
        (uint32_t)(IMU_BSWAP_32(*(uint32_t *)(pBuf + payloadOffset + 30)));
  } else {
    ...
    pRawData->dataCntOrTimeStamp =
        (unsigned)*(uint16_t *)(pBuf + payloadOffset + 28);
    pRawData->crc = (uint32_t)(*(uint32_t *)(pBuf + payloadOffset + 30));
  }
  return Err_imu_Success_e;
}

int adi_imu_ScaleBurstOut_1(adi_imu_Device_t *pDevice, const uint8_t *pBuf,
                            adi_imu_Boolean_e checkBurstID,
                            adi_imu_Boolean_e enByteSwap,
                            adi_imu_BurstOutput_t *pData) {
  unsigned payloadOffset = 0;
  if (checkBurstID) {
    int ret =
        adi_imu_FindBurstPayloadIdx(pBuf, MAX_BRF_LEN_BYTES, &payloadOffset);
  ...
  if (enByteSwap == IMU_TRUE) {
    pData->sysEFlag =
        (unsigned)IMU_BSWAP_16(*(uint16_t *)(pBuf + payloadOffset));
    pData->tempOut =
        getTempOffset(pDevice) +
        (int32_t)(IMU_BSWAP_16(*(uint16_t *)(pBuf + payloadOffset + 2))) *
            getTempRes(pDevice);
    ...
    pData->dataCntOrTimeStamp =
        (unsigned)IMU_BSWAP_16(*(uint16_t *)(pBuf + payloadOffset + 28));
    pData->crc =
        (uint32_t)(IMU_BSWAP_32(*(uint32_t *)(pBuf + payloadOffset + 30)));
  } else {
    pData->sysEFlag = (unsigned)(*(uint16_t *)(pBuf + payloadOffset));
    pData->tempOut = getTempOffset(pDevice) +
                     (int32_t)(*(uint16_t *)(pBuf + payloadOffset + 2)) *
                         getTempRes(pDevice);

    pData->gyro.x =
        (int32_t)(*(uint32_t *)(pBuf + payloadOffset + 4)) * gyroscale;
    pData->gyro.y =
        (int32_t)(*(uint32_t *)(pBuf + payloadOffset + 8)) * gyroscale;
    pData->gyro.z =
        (int32_t)(*(uint32_t *)(pBuf + payloadOffset + 12)) * gyroscale;

    pData->accl.x =
        (int32_t)(*(uint32_t *)(pBuf + payloadOffset + 16)) * acclscale;
    pData->accl.y =
        (int32_t)(*(uint32_t *)(pBuf + payloadOffset + 20)) * acclscale;
    pData->accl.z =
        (int32_t)(*(uint32_t *)(pBuf + payloadOffset + 24)) * acclscale;
...
void adi_imu_ScaleBurstOut_2(adi_imu_Device_t *pDevice,
                             const adi_imu_BurstOutputRaw_t *pRawData,
                             adi_imu_BurstOutput_t *pData) {
...
void adi_imu_ScaleTempOut(adi_imu_Device_t *pDevice, uint16_t rawData,
                          float *pData) {
  *pData = getTempOffset(pDevice) + ((int32_t)rawData) * getTempRes(pDevice);
}

void adi_imu_ScaleAccl32Out(adi_imu_Device_t *pDevice,
                            const adi_imu_AcclOutputRaw32_t *rawData,
                            adi_imu_AcclOutput_t *pOut) {
...
void adi_imu_ScaleGyro32Out(adi_imu_Device_t *pDevice,
                            const adi_imu_GyroOutputRaw32_t *rawData,
                            adi_imu_GyroOutput_t *pOut) {
...
void adi_imu_ScaleAccl16Out(adi_imu_Device_t *pDevice,
                            const adi_imu_AcclOutputRaw16_t *rawData,
                            adi_imu_AcclOutput_t *pOut) {
...
void adi_imu_ScaleGyro16Out(adi_imu_Device_t *pDevice,
                            const adi_imu_GyroOutputRaw16_t *rawData,
                            adi_imu_GyroOutput_t *pOut) {
